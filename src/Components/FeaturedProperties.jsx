...
function FeaturedProperties() {
  ...
  useEffect(() => {
    return () => {
      dispatch(getFeaturedProperties(query));
    };
  }, [dispatch]);

  const FeaturedHotels = useSelector(
    (state) => state.featuredHotelsCountByCityTypesAndQuery
  );
  // console.log("featured", FeaturedHotels.featuredHotels);
  ...
  return (
    <Fragment>
      ...
      >
        {FeaturedHotels.loading && <CircularProgress />}
        {Array.isArray(FeaturedHotels.featuredHotels) &&
          FeaturedHotels.featuredHotels.map((item, idx) => (
            <Box
              key={idx}
              sx={{
                width: "250px",
